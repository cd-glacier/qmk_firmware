...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_INSERT] = LAYOUT( \
      ...
      NORMAL,  _______, _______, _______, _______, KC_SPACE, _______, _______, KC_ENT,  RAISE,  _______, _______, _______, _______ \
      ),
  ...
  switch (keycode) {
    ...
    case RAISE:
      if (record->event.pressed) {
        ...
        layer_on(_RAISE);
        update_tri_layer_RGB(_INSERT, _RAISE, _ADJUST);
      } else {
        #ifdef RGBLIGHT_ENABLE
